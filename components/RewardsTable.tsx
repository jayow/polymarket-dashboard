...
const LIQ_FLOOR = 500 // prevents tiny-liquidity markets from gaming efficiency

const getRewardEfficiency = (m: RewardMarket) =>
  (m.totalDailyRate / Math.max(m.liquidity, LIQ_FLOOR)) * 100

const getSuggestedBids = (m: RewardMarket) => {
...
// --- Grade scoring ---
function percentileRank(val: number, sorted: number[]): number {
...
function computeScore(m: RewardMarket, g: GradeArrays): number {
...
function applyOverround(staticScore: number, ob: OrderbookData | undefined, volume?: number): number {
